...
extractSNR = function (executions) {
    ...
    _.isArray(executions) &&
        _.forEachRight(executions, function (execution) {
...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    colors.disable() // disable colors unconditionally
    ...
    emitter.on("request", function (err, o) {
        ...
        if (!options.verbose) {
            print.lf(
                ...
                o.response.code,
                o.response.reason(),
                util.filesize(o.response.size().total),
                util.prettyms(o.response.responseTime)
            )
        ...
        if (reqTextLen) {
            ...
            if (reqTextLen > BODY_CLIP_SIZE) {
                reqText =
                    reqText.substr(0, BODY_CLIP_SIZE) +
            ...
            print
                .buffer(
                    `  ${colors.white(symbols.console.top)} ${colors.white(symbols.up)} ${reqBodyMode} ${colors.gray(symbols.star)} ${util.filesize(
                        reqTextLen
                    )}\n`,
        ...
        if (resTextLen) {
            ...
            if (resTextLen > BODY_CLIP_SIZE) {
                resText =
                    resText.substr(0, BODY_CLIP_SIZE) +
            ...
            print
                .buffer(
                    `  ${colors.white(symbols.console.top)} ${colors.white(symbols.down)} ${resSummary} ${colors.gray(symbols.star)} ${util.filesize(
                        resTextLen
                    )}\n`,
                    colors.white(`  ${symbols.console.bottom}`)
                )
    ...
    emitter.on("script", function (err, o) {
        err &&
            print.lf(
                colors.red.bold("%s⠄ %s in %s-script"),
                pad(this.summary.run.failures.length, 3, SPC),
                err.name,
                (o.event && o.event.listen) || "unknown"
            )
    })

    !reporterOptions.noAssertions &&
        emitter.on("assertion", function (err, o) {
            ...
            if (reporterOptions.showTimestamps) {
                ...
                print.lf(
                    ...
                    passed ? colors.green(`  ${symbols.ok} `) : colors.red.bold(pad(this.summary.run.failures.length, 2, SPC) + symbols.dot),
                    passed ? colors.gray(o.assertion) : colors.red.bold(o.assertion)
                )
            } else {
                print.lf(
                    "%s %s",
                    passed ? colors.green(`  ${symbols.ok} `) : colors.red.bold(pad(this.summary.run.failures.length, 3, SPC) + symbols.dot),
                    passed ? colors.gray(o.assertion) : colors.red.bold(o.assertion)
                )
            }
        })
    ...
    !reporterOptions.noConsole &&
        emitter.on("console", function (err, o) {
            ...
            message = wrap(
                ...
                `  ${color(symbols.console.middle)} `
            ) // add an indentation line at the beginning
...
_.assignIn(PostmanCLIReporter, {
    ...
    parseFailures(failures) {
        var failureTable = new Table({
            ...
            wordWrap: true,
            colAligns: ["right"],
            colWidths: cliUtils.noTTY()
                ? []
                : (function (size, indexOrder) {
                      var colWidths = []
        ...
        failureTable.push(["", "Failure", "Detail"])
